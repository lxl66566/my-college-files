...
function out = TDLTE_genFredata(inputdata)
    ...
    half = 600; % 表示数据的一半长度，这里设置为 600。
    total = 1200; % 表示数据的总长度，这里设置为 1200。
    out = zeros(14, 2048); % 初始化一个 14 行 2048 列的零矩阵，用于存储生成的频域数据。

    % for (iii = 1:2 * nulsym)：循环从 1 到 2 * nulsym，即从 1 到 14。
    for (iii = 1:2 * nulsym)
        % 从 inputdata 的第 iii 行中提取数据，并将数据分成两部分：
        ...
        % 将生成的 tmpdata 存储到 out 矩阵的第 iii 行。
