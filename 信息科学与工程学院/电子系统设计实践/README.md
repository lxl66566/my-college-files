...
本次实践我全程使用 linux，因此这是一份不太一样的折腾记录；并且用的不是常见发行版（NixOS），可以说可复现度非常低。不过代码都是跨平台兼容的，并且我敢保证代码质量不低：高度模块化 + 提供了易用的用户接口 + 注释完善。

在 windows 上没什么好说的，此处不表。在 linux 上主要考虑编译和烧录两部分。
...
还有 keil 自己的标准：code 关键字表示将不可变结构放到 code 区而不是 ram 区。我一直以为加 const 就是这样优化的，然而并没有。
...
然后我尝试调高中断频率，失败了，只能维持在几十 Hz。猜测是因为中断处理程序耗时较大，导致中断处理过程中多余的中断被忽略。因此在 `motor.c` 里用了个折算技巧。不过实际跑的时候也不会有人管就是了。

### EEPROM

不想了解底层原理，PPT 也没有给代码，好在有学长代码能直接开抄。然而我测试时一直读出的都是 0xFF，对照了半天代码也没发现问题。拿别人板子一测，代码没有问题，芯片是坏的。。。换了芯片就好了。郁闷。

后来调 PID 的时候发现 storage 连续写两个地址，第二个就写不进去，于是又给写操作加了延时，好了。

### 菜单

各模块都写好，万事具备以后我才开始设计菜单和界面。如果模块写得好，这一步的工作量并不算太大……？然后我就发现了菜单设计的问题。
...
本来没想做的，但是看到有两页没写的实验日志就头疼，还是做了一下。（不做也是能过课程的）

首先，让 GPT 写个玩意出来。注意一下，有温度越高，数值越小的 pid（电热器），而我们用的马达是温度越高，数值越大的 pid，因此用 100 减去求得的结果。

然后就是玄学调参，感觉差不多就行了，反正我也不会调（最后检查时说我的参数确实不过关）。遇到了 float +-0.1 导致 +- 1.1 跳变的问题，时间不够不好 debug，于是自己拿两个 U8 模拟了下 float。
...
~~实际前几天写代码都没怎么写，天天折腾环境，不务正业玩 nixos，来实验室偷电偷空调。后几天就赶进度 + 超级 debug 模式，刚做完第二天就有 (jlpt) 考试，压力实在是大。~~

帮同学调代码遇到一些非常神奇的 bug，最神奇的一个是把 `for (j = 0;j < 4;j++);` 改成 `for (j = 4;j > 0; j--);` 就能跑了，j 是 `unsigned char`（[ref](https://t.me/withabsolutex/1727)），实在是太神奇了。所以写代码时尽量注意作用域，能写 .c 就别写在 .h，用 clang 看看警告，生活会很美好。（后来把这位同学半强迫拽到 vscode + clangd 了）

我自己也遇到了不少问题。如何排查问题呢？

1. 拿别人板子试，能复现就是程序的问题，不能复现可能是硬件问题，也有可能程序中出现了未定义行为或非法行为。
2. 拿到单片机给的信息。我一开始用显示屏打地址，后来用串口调试，反正一定要有一个办法接受信息。~~这下 print 大法了~~
3. 直接摆烂换实现，远离一切自己不知道的底层玩意。
